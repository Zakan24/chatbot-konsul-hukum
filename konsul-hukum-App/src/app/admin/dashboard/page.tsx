...
const statCards = [
  { key: 'users', label: 'Total Pengguna', icon: Users, color: 'text-white', bg: 'bg-[#1C2544]' },
  { key: 'chats', label: 'Total Chat', icon: MessageCircle, color: 'text-white', bg: 'bg-[#1C2544]' },
  { key: 'messages', label: 'Total Pesan', icon: FileText, color: 'text-white', bg: 'bg-[#1C2544]' },
  ...
  { key: 'peraturan', label: 'Total Peraturan', icon: Scale, color: 'text-white', bg: 'bg-[#1C2544]' },
] as const
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/80 backdrop-blur-md border border-white/60 rounded-2xl overflow-hidden flex flex-col shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition-all duration-300">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-bold text-gray-800 flex items-center gap-2 text-sm md:text-base">
              <MessageCircle className="w-4 h-4 text-[#D3BA78]" /> Chat Terbaru
            </h3>
            <Link href="/admin/chats" className="text-xs font-medium text-[#D3BA78] hover:text-[#B89E5F] hover:underline">Lihat Semua</Link>
          </div>
                ...
                {recentChatsQuery.data?.items.map(chat => (
                  <div key={chat.id} className="flex items-start justify-between gap-4 border-b border-gray-100/50 last:border-0 pb-4 last:pb-0">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <span className="text-[10px] font-semibold bg-[#D3BA78]/10 text-[#D3BA78] px-2.5 py-0.5 rounded-full inline-block mt-1 border border-[#D3BA78]/20">
        ...
        <div className="bg-white/80 backdrop-blur-md border border-white/60 rounded-2xl overflow-hidden flex flex-col shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition-all duration-300">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href="/admin/feedback" className="text-xs font-medium text-[#D3BA78] hover:text-[#B89E5F] hover:underline">Lihat Semua</Link>
          </div>
          <div className="p-5 flex-1">
